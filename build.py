...
def footer():return '<footer><div class="footer-top">'+brand+'<p>Good websites.<br>Clear direction. Ongoing support.</p><a href="mailto:ryan@websolutionsydney.com.au">Let’s make it happen '+arrow+'</a></div><div class="footer-links"><div><span>WEBSITES</span>'+''.join('<a href="'+p+'">'+n+'</a>' for n,p in website_pages[:4])+'</div><div><span>INDUSTRIES</span>'+''.join('<a href="'+p+'">'+n+'</a>' for n,p in website_pages[4:])+'</div><div><span>GROW & MAINTAIN</span>'+''.join('<a href="'+p+'">'+n+'</a>' for n,p in growth_pages)+'</div><div><span>EXPLORE & CONTACT</span>'+''.join('<a href="'+p+'">'+n+'</a>' for n,p in work_pages+guide_pages)+'<a href="tel:+61420102599">0420 102 599</a><a href="mailto:ryan@websolutionsydney.com.au">ryan@websolutionsydney.com.au</a></div></div><div class="footer-bottom"><span>© 2026 Web Solution Sydney. Established 2017.</span><span>Serving Australian businesses remotely.</span></div></footer>'
...
def portfolio_page():
 cards=''.join('<a class="portfolio-card" href="'+url+'" target="_blank" rel="noopener noreferrer"><div class="portfolio-card-top"><span>'+num+' /</span><span>'+html.escape(category.upper())+'</span></div><h2>'+html.escape(name)+'</h2><p>'+html.escape(category)+' website</p><span class="portfolio-open">Open live site '+arrow+'</span></a>' for num,name,category,url in portfolio_projects)
...
favicon="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='15' fill='%2339c7ef'/%3E%3Cpath d='M10 22l8 25 13-21 10 21 13-30' fill='none' stroke='%23071c2d' stroke-width='7'/%3E%3C/svg%3E"
def seo_head(p):
...
for p in pages:
 document='<!doctype html><html lang="en-AU"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#e5f7fc"><title>'+html.escape(p['title'])+'</title><meta name="description" content="'+html.escape(p['meta'],quote=True)+'"><link rel="canonical" href="https://websolutionsydney.com.au'+p['url']+'">'+seo_head(p)+'<link rel="icon" href="'+favicon+'"><link rel="stylesheet" href="/styles.css"><script src="/site.js" defer></script></head><body><a class="skip" href="#content">Skip to content</a>'+header(p['url'])+'<div id="content">'+(home_with_industries() if p['url']=='/' else inner(p))+'</div>'+footer()+'</body></html>'
