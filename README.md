...
Set `PUBLIC_SITE_URL` to the exact public HTTPS origin. Set `ENQUIRY_TO` to the owner's confirmed inbox and `ENQUIRY_FROM` to an address on a domain verified in Resend. Configure `RESEND_API_KEY`, `TURNSTILE_SECRET_KEY` as secrets and `TURNSTILE_SITE_KEY` as the public widget key. The widget must allow the public hostname. Do not paste secret values into chat or commit them.
